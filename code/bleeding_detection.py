...
class BleedingDetectorNode:
    def __init__(self):
        rospy.init_node("bleending_detector_node")

        # ---- Params base ----
        self.mode = str(rospy.get_param("~mode", "C1")).strip().upper()
        if self.mode not in VALID_MODES:
            rospy.logwarn(f"[bleending] ~mode='{self.mode}' inválido. Usando C1.")
            self.mode = "C1"
        ...
        rospy.loginfo(f"[bleending] mode={self.mode} show_age={self.show_age} show_detection={self.show_detection}")
        rospy.loginfo(f"[bleending] mask_morph={self.use_mask_morph} mask_kernel={self.MASK_KERNEL}")
        rospy.loginfo(f"[bleending] alpha2d={self.ALPHA_FILTER_2D} growth_gate={self.use_growth_gate} min_growth_area={self.MIN_GROWTH_AREA}")
        rospy.loginfo(f"[bleending] C3 clamp: w_min={self.C3_W_MIN} w_max={self.C3_W_MAX}")
        rospy.loginfo(f"[bleending] C2-core: percentile={self.C2_CORE_PERCENTILE} min_area={self.C2_CORE_MIN_AREA}")
        rospy.loginfo(f"[bleending] hold_target_time={self.hold_target_time}")

        # ---- Modelo ----
        self.modelo_sangre = load_model(MODEL_PATH, compile=False)

        # ---- ROS pubs ----
        self.pub_centroids_2d = rospy.Publisher("/bleending/centroids_2d", Float32MultiArray, queue_size=1)
        self.pub_centroides   = rospy.Publisher("/bleending/sangre_centroides", Point, queue_size=10)
        self.pub_punto_unico  = rospy.Publisher("/bleending/punto_objetivo", Point, queue_size=1)
        self.pub_imagen       = rospy.Publisher("/bleending/image_overlay", Image, queue_size=1)
        self.pub_sangre_ok    = rospy.Publisher("/bleending/sangre_ok", Bool, queue_size=1)
    ...
    def publish_and_draw(self, display_img, puntos_actuales, selected_blob, labels, depth_frame):
        ...
        if xyz is None:
            rospy.logwarn_throttle(1.0, "[bleending] Depth inválido. Reutilizando último target válido.")
            self.publish_last_valid_target_if_recent()
            return
